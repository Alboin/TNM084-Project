...
vec3 grassOnTop()
{
  ...
  float noise6 = (snoise(newPos * 60) * 0.5 + 0.5) * 0.2 + 0.8;
	...
	vec3 brown = vec3(0.9, 0.7, 0.5) * noise0 * noise1 * noise2 * noise3 * noise4 * noise5 * noise6;
	vec3 green = vec3(0.55, 1.0, 0.55) * noise0 * noise1 * noise2 * noise3 * noise4 * noise5 * noise6;
